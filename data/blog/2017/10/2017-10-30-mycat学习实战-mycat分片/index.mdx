...
- `partitionLength`为分片范围列表。 分区长度:默认为最大 `2^n=1024` ,即最大支持 1024 分区 约束 : count,length 两个数组的长度必须是一致的。 `1024 = sum((count[i]*length[i]))` count 和 length 两个向量的点积恒等于 1024 本例的分区策略：希望将数据水平分成 3 份，前两份各占 25%，第三份占 50%。（故本例非均匀分区）
